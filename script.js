...
class CasinoBalance {
    ...
    static setBalance(amount) {
        ...
        // Reset auto winnings display if auto mode is disabled
        if (window.plinkoGame && !window.plinkoGame.isAutoMode) {
            // Save the final net amount before clearing
            const finalNetAmount = window.plinkoGame.amountWon - window.plinkoGame.amountSpent;
            window.plinkoGame.updateLastAutoNet(finalNetAmount);
            
            window.plinkoGame.amountSpent = 0;
            window.plinkoGame.amountWon = 0;
            window.plinkoGame.accumulatedWinnings = 0;
            window.plinkoGame.updateWinningsDisplay();
        }
...
class BlackjackGame {
    ...
    async hit() {
        ...
        // Disable hit and stand buttons immediately to prevent spam clicking
        this.hitBtn.disabled = true;
        this.standBtn.disabled = true;
        ...
        if (this.splitHands) {
            ...
            if (this.calculateHandValue(this.splitHands[this.splitActiveHand]) > 21) {
                ...
                // Re-enable hit button if split hand didn't bust
                this.hitBtn.disabled = false;
                this.standBtn.disabled = false;
            }
            this.updateSplitButtonState();
        ...
        if (this.calculateHandValue(this.playerHand) > 21) {
            ...
            // Re-enable hit and stand buttons if player didn't bust
            this.hitBtn.disabled = false;
            this.standBtn.disabled = false;
        }
        this.updateSplitButtonState();
    ...
    async playSplitHands(isAceSplit) {
        ...
        // If ace split, auto-stand both hands and proceed to dealer
        if (isAceSplit) {
            ...
            // Automatically proceed to dealer play after a short delay
            setTimeout(async () => {
                await this.finishSplitHands();
            }, 1500);
            return;
        }
...
class PlinkoGame {
    constructor() {
        ...
        this.amountSpent = 0; // Track total amount spent on auto drops
        this.amountWon = 0; // Track total amount won from auto drops
        ...
        this.hasUnresolvedBalls = false; // Track if there are balls still falling in manual mode
        this.topNotificationTimeout = null; // Timeout for top notification auto-hide
    ...
    setMode(mode) {
        // Prevent switching to auto mode if there are unresolved balls in manual mode
        if (mode === 'auto' && !this.isAutoMode && this.hasUnresolvedBalls) {
        ...
        if (!this.isAutoMode) {
            ...
            this.amountWon = 0;
            this.updateWinningsDisplay();
            // Hide auto winnings container when switching to manual mode
            if (this.winningsContainer) {
                this.winningsContainer.style.display = 'none';
            }
        }
    ...
    updateAutoTabDisabledState() {
        if (this.autoTab) {
            // Disable auto tab if we're in manual mode and have unresolved balls
            if (!this.isAutoMode && this.hasUnresolvedBalls) {
    ...
    showTopNotification(message, duration = 3000) {
        const topNotification = document.getElementById('plinko-top-notification');
        if (topNotification) {
    ...
    hideTopNotification() {
        ...
        if (this.topNotificationTimeout) {
            clearTimeout(this.topNotificationTimeout);
            this.topNotificationTimeout = null;
        }
    }
    ...
    dropBall() {
        ...
        // Track amount spent in auto mode - use the actual bet amount for this ball
        if (this.isAutoMode) {
            this.amountSpent += this.bet;
            this.updateWinningsDisplay(); // Update display immediately when ball is dropped
        }
        ...
        // Mark that there are unresolved balls when dropping in manual mode
        if (!this.isAutoMode) {
            this.hasUnresolvedBalls = true;
            this.updateAutoTabDisabledState();
        }
    }
    ...
    update() {
        ...
        // Check if all balls are resolved in manual mode
        if (!this.isAutoMode && this.balls.length === 0) {
            this.hasUnresolvedBalls = false;
            this.updateAutoTabDisabledState();
            this.hideTopNotification(); // Hide any "wait for balls" message
        }
        
        // Check if all balls are resolved in auto mode and handle balance checking
        if (this.isAutoMode && this.balls.length === 0) {
            ...
            if (this.autoPausedForFunds || this.pendingAutoPayout) {
                ...
                // Get fresh balance after payout
                this.balance = CasinoBalance.getBalance();
                
                if (this.balance >= this.bet && this.shouldResumeAfterPayout && !this.userRequestedStop) {
                    // Resume auto dropping
                    this.restartAutoInterval();
                } else {
                    // Stop auto dropping - insufficient funds or user requested stop
                    this.stopAutoDrop();
                }
            }
        }
    ...
    updateWinningsDisplay() {
        ...
        if (amountSpentElement) {
            amountSpentElement.textContent = Math.floor(this.amountSpent).toLocaleString();
        }
        
        // Update amount won
        const amountWonElement = document.getElementById('amount-won');
        if (amountWonElement) {
            amountWonElement.textContent = Math.floor(this.amountWon).toLocaleString();
        }
        
        // Update net win
        const netWinElement = document.getElementById('net-win');
        if (netWinElement) {
            const netWin = this.amountWon - this.amountSpent;
            const isPositive = netWin >= 0;
            netWinElement.textContent = (isPositive ? '+' : '') + Math.floor(netWin).toLocaleString();
            netWinElement.className = 'net-win ' + (isPositive ? 'positive' : 'negative');
        }
        
        // Update last auto net display visibility
        this.updateLastAutoNetVisibility();
    }

    updateLastAutoNet(netAmount) {
        const lastAutoElement = document.getElementById('last-auto-amount');
        if (lastAutoElement) {
            if (netAmount === null || netAmount === undefined) {
                lastAutoElement.textContent = 'N/A';
            } else {
                const isPositive = netAmount >= 0;
                lastAutoElement.textContent = (isPositive ? '+' : '') + Math.floor(netAmount).toLocaleString();
                lastAutoElement.style.color = isPositive ? '#44ff44' : '#ff4444';
            }
        }
    }

    updateLastAutoNetVisibility() {
        const lastAutoContainer = document.getElementById('last-auto-net');
        if (lastAutoContainer) {
            const shouldShow = this.isAutoMode || this.accumulatedWinnings > 0 || this.autoPausedForFunds || this.pendingAutoPayout;
            lastAutoContainer.style.display = shouldShow ? 'flex' : 'none';
        }
    }
    ...
    stopAutoDrop() {
        ...
        // Save the final net amount before clearing
        const finalNetAmount = this.amountWon - this.amountSpent;
        this.updateLastAutoNet(finalNetAmount);
        
        // Clear auto winnings display when auto mode ends
        this.amountSpent = 0;
        this.amountWon = 0;
        this.accumulatedWinnings = 0;
        this.updateWinningsDisplay();
...
document.addEventListener('keydown', (event) => {
    if (event.key.toLowerCase() === 'r' && !rKeyHeld) {
        rKeyHeld = true;
        rKeyBalance = CasinoBalance.getBalance();
    }
    
    if (event.key.toLowerCase() === 'r' && rKeyHeld) {
        // Add 5000 to local balance while key is held (localStorage only, no Firebase)
        rKeyBalance += 5000;
        localStorage.setItem('casinoBalance', rKeyBalance);
        
        // Update UI displays without triggering Firebase save
        const balanceElements = document.querySelectorAll('.balance-amount, .coin-balance');
        balanceElements.forEach(element => {
            element.textContent = Math.floor(rKeyBalance);
        });
        
        console.log('Balance updated via R key (held). New balance:', rKeyBalance);
    }
});
